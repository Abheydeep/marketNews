...
- 2026-07-01: Fixed archived-slot publishing across the midnight boundary before release. ROOT CAUSE: `run()` in `tools/vercel-build-public.mjs` invoked the compound `site:publish` npm script with source date/time arguments, but they reached only the second command while `publish-site.mjs` defaulted to the current date, causing code-only deployments after midnight to select the wrong archive and fail artifact validation.
  Verified: rollover regression test passed; `npm run context:verify` passed; `npm test` passed 86 repository tests and 31 Node tests; `npm run test:deploy` passed all 5 checks in 18.5 seconds, including the public Vercel artifact and 18 FastAPI tests; `npm run public:copy:qa -- public` passed.
  Architecture diagrams changed: none.
